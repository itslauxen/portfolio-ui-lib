...
// LiquidEther: simulacao de fluido (Navier-Stokes em GPU) com three.js.
// O mouse injeta forca no campo de velocidade; um piloto automatico move o
// cursor quando o usuario esta inativo. O contexto GL e criado uma unica vez
// (useEffect com deps vazias) e um segundo efeito sincroniza as opcoes da
// simulacao ao vivo quando as props mudam, sem recriar nada.
...
// Cria a textura 1D da paleta de cores usada para colorir a velocidade.
...
export default function LiquidEther({
  ...
  // Ref com as props atuais (usada na montagem unica do contexto GL).
  ...
  useEffect(() => {
    ...
    let paletteTex = makePaletteTexture(init.colors);
    colorsKeyRef.current = init.colors.join(",");
    // Fundo transparente fixo (alfa 0).
    ...
    // Piloto automatico: passeia o "cursor" quando o usuario esta inativo.
    class AutoDriver {
      mouse: MouseClass;
...
`;
    ...
    // Passo generico de shader renderizando num render target.
    class ShaderPass {
      props: any;
      uniforms?: Uniforms;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        const cursorSizeValue = props.cursor_size || 0;
        const cursorSizeX = cursorSizeValue * cellScale.x;
        const cursorSizeY = cursorSizeValue * cellScale.y;
        ...
        uniforms.scale.value.set(cursorSizeValue, cursorSizeValue);
        super.update();
      }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      // Troca a paleta de cores ao vivo, sem recriar a simulacao.
      setPalette(stops: string[]) {
        const old = paletteTex;
        paletteTex = makePaletteTexture(stops);
        if (this.output && this.output.material) {
    ...
    const webgl = new WebGLManager({
      $wrapper: container,
      autoDemo: init.autoDemo,
      autoSpeed: init.autoSpeed,
      autoIntensity: init.autoIntensity,
      takeoverDuration: init.takeoverDuration,
      autoResumeDelay: init.autoResumeDelay,
      autoRampDuration: init.autoRampDuration,
    });
    webglRef.current = webgl;

    // Aplica as opcoes iniciais da simulacao a partir das props.
    const sim = webgl.output?.simulation;
    if (sim) {
      const prevRes = sim.options.resolution;
      Object.assign(sim.options, {
        mouse_force: init.mouseForce,
        cursor_size: init.cursorSize,
        isViscous: init.isViscous,
        viscous: init.viscous,
        iterations_viscous: init.iterationsViscous,
        iterations_poisson: init.iterationsPoisson,
        dt: init.dt,
        BFECC: init.BFECC,
        resolution: init.resolution,
        isBounce: init.isBounce,
      });
      if (init.resolution !== prevRes) sim.resize();
    }
    webgl.start();
    ...
    // Montagem unica: as mudancas de props sao aplicadas pelo efeito de sincronizacao abaixo.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Sincroniza as opcoes da simulacao, o piloto automatico e a paleta ao vivo.
  useEffect(() => {
    ...
    if (key !== colorsKeyRef.current) {
      colorsKeyRef.current = key;
      webgl.setPalette?.(colors);
    }
