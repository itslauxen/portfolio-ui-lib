...
// PlasmaWave, ondas de plasma em raymarch, shader WebGL (OGL).
// Componente TypeScript + CSS Module. Requer `ogl`.
import { useRef, useEffect } from "react";
import { Renderer, Camera, Transform, Program, Mesh, Geometry } from "ogl";
import styles from "./PlasmaWave.module.css";
...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      const cols = cur.colors ?? ["#A855F7", "#06B6D4"];
      uniformOffset[0] = cur.xOffset ?? 0;
      uniformOffset[1] = cur.yOffset ?? 0;
      program.uniforms.iTime.value = (now - startTime) * 0.001;
      program.uniforms.uRotation.value = ((cur.rotationDeg ?? 0) * Math.PI) / 180;
      program.uniforms.uFocalLength.value = cur.focalLength ?? 0.8;
      program.uniforms.uSpeed1.value = cur.speed1 ?? 0.05;
      program.uniforms.uSpeed2.value = cur.speed2 ?? 0.05;
      program.uniforms.uDir2.value = cur.dir2 ?? 1.0;
      program.uniforms.uBend1.value = cur.bend1 ?? 1;
      program.uniforms.uBend2.value = cur.bend2 ?? 0.5;
