...
// ColorBends, faixas de cor organicas que se dobram e fluem (three.js).
// O contexto GL e criado uma unica vez; mudancas de props atualizam uniforms ao vivo.
...
uniform vec2 uPointer; // em NDC [-1,1]
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            float kMix = pow(kBelow, 0.3); // resposta forte no intervalo 0..1
            float gain = 1.0 + max(uWarpStrength - 1.0, 0.0); // acima de 1 amplifica o deslocamento
...
export default function ColorBends({
  ...
  // Criacao unica do renderer/cena; nada aqui depende de props (elas entram via refs/uniforms).
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Rotacao (fixa + automatica) recalculada a cada frame a partir dos refs.
      ...
      // Suavizacao do ponteiro (lerp em direcao ao alvo).
  ...
  // Mudancas de props atualizam uniforms ao vivo, sem recriar o contexto GL.
  useEffect(() => {
  ...
  // Rastreia o ponteiro em coordenadas NDC do container.
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
