...
// Pintura metalica liquida em WebGL2: uma mascara e preenchida com listras
// metalicas que escorrem e refratam. Em vez de carregar uma imagem externa,
// a mascara e gerada proceduralmente num canvas fora da tela (texto em negrito),
// e um campo de profundidade e resolvido por relaxamento (SOR) sobre a forma.
...
export interface MetallicPaintProps {
  /** Texto usado como mascara (desenhado em negrito num canvas fora da tela). */
  text?: string;
  seed?: number;
  scale?: number;
  refraction?: number;
  ...
  /** Quando ativo, o movimento segue o cursor em vez do tempo. */
...
// Desenha o texto em negrito num canvas quadrado transparente. O alpha do
// desenho vira a mascara da forma no shader.
function gerarCanvasDeTexto(texto: string, tamanho: number): HTMLCanvasElement {
  ...
  let fontSize = tamanho * 0.5;
  ctx.font = `900 ${fontSize}px ${familia}`;
  const largura = ctx.measureText(texto).width;
  const maxLargura = tamanho * 0.82;
  if (largura > maxLargura) {
    fontSize = (fontSize * maxLargura) / largura;
  }
  ctx.font = `900 ${fontSize}px ${familia}`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = "#000000";
  ctx.fillText(texto, tamanho / 2, tamanho / 2 + fontSize * 0.04);

  return canvas;
}

// Constroi a textura de profundidade: resolve um campo suave dentro da forma
// por relaxamento sucessivo (SOR), com a borda como condicao de contorno.
function processarMascara(mask: HTMLCanvasElement): ImageData {
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    const side = Math.round(700 * Math.min(window.devicePixelRatio || 1, 2));
    canvas.width = side;
    canvas.height = side;
  ...
  // Gera a mascara procedural e envia como textura.
  useEffect(() => {
    if (!ready) return;

    setTextureReady(false);
    const mask = gerarCanvasDeTexto(text || "GL", 600);
    const imgData = processarMascara(mask);
    uploadTexture(imgData);
    setTextureReady(true);
  }, [ready, text, uploadTexture]);
  ...
  useEffect(() => {
    ...
    // Movimento pelo cursor fica restrito ao proprio canvas.
  ...
  return <canvas ref={canvasRef} className={styles.paintContainer} />;
}
