...
// Texto 3D renderizado como arte ASCII: um plano com o texto e distorcido
// por shaders (three.js) e o resultado vira caracteres em um <pre>.
// Adaptado e aprimorado a partir de https://codepen.io/JuanFuentes/pen/eYEeoyE
...
// Fonte mono usada no texto e na grade ASCII, carregada uma unica vez.
const FONT_HREF = "https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@500;600&display=swap";
const FONT_LINK_ID = "fonte-ibm-plex-mono-ascii";
...
// Converte o resultado do renderer WebGL em caracteres dentro de um <pre>.
class AsciiFilter {
...
// Desenha o texto em um canvas 2D que vira textura do plano 3D.
class CanvasTxt {
  canvas: HTMLCanvasElement;
  context: CanvasRenderingContext2D | null;
  txt: string;
...
// Orquestra cena, camera, plano com shader e o filtro ASCII.
class CanvAscii {
  textString: string;
  asciiFontSize: number;
  textFontSize: number;
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      if (width === 0 || height === 0) {
        // Espera o container ganhar tamanho antes de criar a cena.
